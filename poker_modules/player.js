 ...
 */
const Player = function (socket, id, name, chips) {
    this.public = {
        // The id of the user
        id: id,
...
Player.prototype.evaluateHand = function (board) {
    const cards = this.cards.concat(board)
    ...
    const getCardName = function (cardValue, plural) {
    ...
    const swap = function (index1, index2) {
        if (cardNamess.indexOf(cards[index1][0]) < cardNamess.indexOf(cards[index2][0])) {
            const tmp = cards[index1]
            cards[index1] = cards[index2]
            cards[index2] = tmp
        }
    }

    const rateHand = function (hand) {
    ...
    for (var i in flushes) {
        const flushLength = flushes[i].length
        if (flushLength >= 5) {
    ...
    if (!evaluatedHand.rank) {
        ...
        if (numberOfPairs) {
            ...
            // If there are two pairs
                ...
                const pairKeys = [Object.keys(pairs)[0], Object.keys(pairs)[1], Object.keys(pairs)[2]]
