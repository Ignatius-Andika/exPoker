 ...
 */
const Table = function (id, name, eventEmitter, seatsCount, bigBlind, smallBlind, maxBuyIn, minBuyIn) {
...
Table.prototype.initializeRound = function (changeDealer) {
    ...
    if (this.playersSittingInCount > 1) {
        ...
        if (this.public.dealerSeat === null) {
            const randomDealerSeat = Math.ceil(Math.random() * this.playersSittingInCount)
            let playerCounter = 0
            var i = -1
...
Table.prototype.initializePreflop = function () {
    // 
    console.log('INITIALIZE PREFLOP');
    this.stopTimeoutSB()
    this.stopTimeoutBB()
    this.stopTimeoutFolded()
    // 
...
Table.prototype.showdown = function () {
    ...
    const messages = this.pot.destributeToWinners(this.seats, currentPlayer)
...
Table.prototype.playerPostedSmallBlind = function () {
    const bet = this.seats[this.public.activeSeat].public.chipsInPlay >= this.public.smallBlind ? this.public.smallBlind : this.seats[this.public.activeSeat].public.chipsInPlay
...
Table.prototype.playerPostedBigBlind = function () {
    const bet = this.seats[this.public.activeSeat].public.chipsInPlay >= this.public.bigBlind ? this.public.bigBlind : this.seats[this.public.activeSeat].public.chipsInPlay
    this.seats[this.public.activeSeat].bet(bet)
    this.log({
...
Table.prototype.playerRaised = function (amount) {
    // 
    this.raised += 1
    console.log('PLAYER RAISED : ', this.raised);
    // 
    ...
    this.public.biggestBet = this.public.biggestBet < this.seats[this.public.activeSeat].public.bet ? this.seats[this.public.activeSeat].public.bet : this.public.biggestBet
    const raiseAmount = this.public.biggestBet - oldBiggestBet
    this.log({
...
Table.prototype.timeoutFolded = function () {
    this.timeoutFold = setTimeout(() => {
        this.playerFolded()
        this.stopTimeoutFolded()
        // console.log('TIMEOUT FOLDED EXECUTE');
    }, 20000)
    
}

// 
Table.prototype.timeoutSB = function () {
    this.timeoutSmallB = setTimeout(() => {
        this.playerPostedSmallBlind()
        this.stopTimeoutSB()
        // console.log('TIMEOUT SMALL BLIND EXECUTE');
    }, 20000)
}

Table.prototype.timeoutBB = function () {
    this.timeoutBigB = setTimeout(() => {
        this.playerPostedBigBlind()
        this.stopTimeoutBB()
        // console.log('TIMEOUT BIG BLIND EXECUTE');
    }, 20000)
}

Table.prototype.stopTimeoutSB = function () {
    clearTimeout(this.timeoutSmallB)
    // console.log('CLEAR TIMEOUT SMALL BLIND');
}

Table.prototype.stopTimeoutBB = function () {
    clearTimeout(this.timeoutBigB)
    // console.log('CLEAR TIMEOUT BIG BLIND');
}
// 

/**
 * Method to stops timeout folded
 */
Table.prototype.stopTimeoutFolded = function () {
    clearTimeout(this.timeoutFold)
    // console.log('CLEAR TIMEOUT FOLDED');
}
